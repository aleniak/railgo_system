...
function explainLabel(v) {
  if (v >= 0.75) return "очень высокий";
  if (v >= 0.55) return "повышенный";
  if (v >= 0.35) return "умеренный";
  return "низкий";
}

function activeRisk(w) {
  return state.horizon === "14" ? (w.risk14d ?? 0) : (w.risk90d ?? 0);
}

function activePred(w) {
  return state.horizon === "14" ? w.pred14d : w.pred90d;
}

function activeDrivers(w) {
  return state.horizon === "14" ? (w.drivers14d || []) : (w.drivers90d || []);
}

function activeGroups(w) {
  return state.horizon === "14" ? (w.groupDrivers14d || []) : (w.groupDrivers90d || []);
}

function activeShap(w) {
  if (state.horizon === "14") {
    return w.shap14d || w.shap_14d || [];
  }
  return w.shap90d || w.shap_90d || [];
}
...
function showFatalError(message, details = "") {
  ...
  content.innerHTML = `
    <section class="card" style="border:1px solid #f2b8b5; background:#fff7f7;">
      <h2 style="margin-bottom:10px; color:#b42318;">Ошибка загрузки данных</h2>
...
function renderSummary() {
  ...
  document.getElementById("summaryCards").innerHTML = `
    <div class="mini-card"><span>Всего вагонов</span><strong>${s.totalWagons ?? "—"}</strong></div>
    <div class="mini-card"><span>Локаций</span><strong>${s.locations ?? "—"}</strong></div>
    <div class="mini-card"><span>Высокий риск 14д</span><strong>${s.highRisk14d ?? "—"}</strong></div>
    <div class="mini-card"><span>Высокий риск 90д</span><strong>${s.highRisk90d ?? "—"}</strong></div>
  `;
}
...
function filteredWagons() {
  return state.data.wagons.filter(w => {
    ...
    const countryFrom = String(w.countryFrom ?? "").toLowerCase();

    const matchesQ =
      !q ||
      wagonId.includes(q) ||
      location.includes(q) ||
      countryFrom.includes(q);
...
function groupedTree() {
  ...
  for (const w of filteredWagons()) {
    ...
    if (!tree[country].stations[station]) {
      tree[country].stations[station] = [];
    }
...
function stationKey(country, station) {
  return `station::${country}::${station}`;
}

function ensureCollapsedDefaults() {
  for (const [country, info] of groupedTree()) {
    const ck = countryKey(country);
    if (!(ck in state.collapsedCountries)) {
      state.collapsedCountries[ck] = true;
    }

    for (const station of Object.keys(info.stations)) {
      const sk = stationKey(country, station);
      if (!(sk in state.collapsedStations)) {
        state.collapsedStations[sk] = true;
      }
    }
  }
}
...
function renderLocations() {
  ...
  for (const [country, info] of tree) {
    ...
    cHeader.innerHTML = `
      ...
      <span class="count-badge">${info.count} вагонов</span>
    `;
    ...
    for (const [station, wagons] of stations) {
      ...
      sHeader.innerHTML = `
        ...
        <span class="count-badge">${wagons.length}</span>
      `;
      sHeader.onclick = (e) => {
        e.stopPropagation();
        toggleStation(country, station);
      };
      ...
      wagons.forEach(w => {
        ...
        btn.innerHTML = `
          ...
          <div class="wagon-route">${station}</div>
          <div class="wagon-meta">
            <span>${state.horizon}д ${fmtPct(activeRisk(w))}</span>
            <span>${activePred(w) ? "риск" : "низкий риск"}</span>
          </div>
        `;
  ...
  if (!drivers.length && !groups.length) {
    return `
      <div class="explain-box">
        <div class="explain-title">Почему такой прогноз</div>
        <p class="muted">
          Модель оценивает риск как <strong>${explainLabel(risk)}</strong>,
          но для этой записи нет прикрепленного текстового объяснения.
        </p>
      </div>
    `;
  }
  ...
  return `
    <div class="explain-box">
      ...
      <p class="explain-text">
        Для горизонта <strong>${state.horizon} дней</strong> модель оценивает риск поломки как
        <strong>${explainLabel(risk)}</strong> (${fmtPct(risk)}).
      </p>
      <p class="explain-text">
        Наиболее важные прикрепленные факторы:
        <strong>${topDriverNames || "нет данных"}</strong>.
      </p>
      <p class="explain-text">
        На уровне групп факторов прогноз в основном связан с:
        <strong>${topGroups || "нет данных"}</strong>.
      </p>
    </div>
  `;
}
...
function renderShapChart(items) {
  ...
  const normalized = items
    .map(item => ({
      ...
      prettyValue: item.prettyValue ?? item.pretty_value ?? item.raw_value ?? ""
    }))
    .filter(x => !Number.isNaN(x.value));

  if (!normalized.length) {
    root.innerHTML = `<div class="empty-box">SHAP-данные есть, но их формат не распознан.</div>`;
    return;
  }

  const maxVal = Math.max(...normalized.map(x => Math.abs(x.value)), 0.0001);

  root.innerHTML = normalized.map(item => {
    const width = Math.max(6, Math.round(Math.abs(item.value) / maxVal * 100));
    return `
      <div class="shap-bar-row">
        <div class="shap-label" title="${item.feature}">${item.feature}</div>
        <div class="shap-bar-track">
          <div class="shap-bar-fill" style="width:${width}%"></div>
        </div>
        <div class="shap-number">${item.value >= 0 ? "+" : ""}${item.value.toFixed(4)}</div>
      </div>
    `;
  }).join("");
}
...
function renderDetails() {
  ...
  document.getElementById("contextList").innerHTML = [
    kv("Страна отправления", countryOf(w)),
    kv("Станция отправления", stationOf(w)),
    kv("Погода", w.weather),
  ...
  document.getElementById("driversMain").innerHTML =
    buildNarrative(w) +
    (activeDrivers(w).length ? activeDrivers(w).map(driverCard).join("") : `<p class="muted">Нет прикрепленных факторов.</p>`);

  document.getElementById("groupsMain").innerHTML =
    activeGroups(w).length ? activeGroups(w).map(groupCard).join("") : `<p class="muted">Нет групп факторов.</p>`;
...
async function loadData() {
  try {
    ...
    if (!data || typeof data !== "object") {
      throw new Error("JSON пустой или битый.");
    }

    if (!data.summary || !Array.isArray(data.wagons)) {
      throw new Error("В JSON должны быть ключи summary и wagons.");
    }
