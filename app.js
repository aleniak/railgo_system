...
function highRiskCount(){
  ...
  return state.horizon === "14" ? (s.highRisk14d ?? "—") : (s.highRisk90d ?? "—");
}
...
function renderSummary(){
  const s=state.data.summary;
  const riskLabel = state.horizon === "14" ? "Высокий риск 14д" : "Высокий риск 90д";
  document.getElementById('summaryCards').innerHTML=`
    ...
    <div class="mini-card"><span>${riskLabel}</span><strong>${highRiskCount()}</strong></div>
  `;
}

function countryOf(w){ return w.countryFrom || "Неизвестная страна"; }
function stationOf(w){ return w.location || "Неизвестная станция"; }
  ...
  for(const [country, info] of tree){
    ...
    for(const [station, wagons] of stations){
      ...
      wagons.forEach(w=>{
        ...
        btn.className='wagon-item'+(state.selected&&String(state.selected.wagonId)===String(w.wagonId)?' active':'');
...
function selectWagon(id){
  state.selected=state.data.wagons.find(w=>String(w.wagonId)===String(id));
  renderLocations();
  renderDetails();
}
...
function renderShapChart(items){
  ...
  root.innerHTML = normalized.map(item => {
    ...
    const pct = val * 100;
    return `<div class="shap-bar-row"><div class="shap-label" title="${item.feature ?? ''}">${item.feature ?? '—'}</div><div class="shap-bar-track"><div class="shap-bar-fill" style="width:${width}%"></div></div><div class="shap-number">${pct >= 0 ? '+' : ''}${pct.toFixed(1)}%</div></div>`;
  }).join('');
}
...
function updateHorizonTexts(){
  ...
  document.getElementById('factorsTitle').textContent = `SHAP-факторы модели · ${h} дней`;
...
function renderDetails(){
  ...
  if(!w){
    ...
    document.getElementById('wagonSubtitle').textContent='Нажмите на вагон слева, чтобы посмотреть прогноз и SHAP-факторы модели.';
